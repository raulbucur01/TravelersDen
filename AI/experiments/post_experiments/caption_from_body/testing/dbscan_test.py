...
def get_dbscan_similar_posts(
    post_id=None,  # Allow None to pick a random valid ID
    clustered_posts_path="../../caption_from_body/posts_dbscan_cosine.csv",
...
similar_posts = get_dbscan_similar_posts(1356)

# Display similar posts
if similar_posts is not None:
    print("\nPosts similar to the selected post based on DBSCAN clustering:")
    print(similar_posts)
